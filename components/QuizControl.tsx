...
const QuizControl: React.FC<Props> = ({ activeEvent, lang }) => {
  ...
  // --- СИНХРОНИЗАЦИЯ: Как только вы что-то меняете локально, это летит в Firebase ---
  useEffect(() => {
    // 1. Сохраняем в localStorage (как и было)
    localStorage.setItem('game_state', JSON.stringify({
      gameType: gameMode, currentIdx, questStage, isActive: currentIdx >= 0 || countdown !== null,
      isCountdown: countdown !== null, countdownValue: countdown, questions, artTheme, timestamp: Date.now()
    }));

    // 2. ОТПРАВЛЯЕМ В FIREBASE (Новое!)
    FirebaseService.syncGameState({
      gameType: gameMode, currentIdx, questStage, isActive: currentIdx >= 0 || countdown !== null,
      isCountdown: countdown !== null, countdownValue: countdown, questions, artTheme, code: activeEvent.code, timestamp: Date.now()
    });
