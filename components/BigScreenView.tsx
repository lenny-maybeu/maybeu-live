...
import { FirebaseService } from '../services/firebase'; // <--- Добавил импорт
...
const BigScreenView: React.FC<Props> = ({ lang }) => {
  ...
  useEffect(() => {
    // ПОДПИСЫВАЕМСЯ НА ВСЁ ИЗ ОБЛАКА
    FirebaseService.subscribeToEverything(
        (event) => { /* Можно реагировать на событие */ },
        (game) => setGameState(game),
        (count) => setOnlineCount(count)
    );

    FirebaseService.subscribeToRace(val => val && setRaceProgress(val));
    FirebaseService.subscribeToImages(val => val && setImages(Object.values(val)));
  }, []);

  if (!gameState || !gameState.isActive) {
     return (
        <div className="flex-1 bg-slate-950 flex flex-col items-center justify-center text-center h-screen">
           <h1 className="text-6xl font-black text-white uppercase italic tracking-widest mb-8">MAYBEU LIVE</h1>
           <div className="flex items-center gap-4 text-2xl text-slate-500 font-bold uppercase justify-center"><Users /> {onlineCount} ONLINE</div>
  ...
  return (
    <div className="flex-1 bg-slate-950 p-12 flex flex-col relative overflow-hidden h-screen">
          ...
          {gameState.gameType === GameType.IMAGE_GEN && (
             <div className="grid grid-cols-4 gap-4">
                {images.slice(-8).map((img, i) => (
                   <div key={i} className="aspect-square rounded-3xl overflow-hidden border-4 border-white"><img src={img.url} className="w-full h-full object-cover" /></div>
                ))}
             </div>
          )}
       </div>
