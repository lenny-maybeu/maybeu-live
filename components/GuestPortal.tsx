...
const GuestPortal: React.FC<Props> = ({ lang }) => {
  ...
  const handleJoin = async () => {
     ...
     // !!! ГЛАВНОЕ ИЗМЕНЕНИЕ: ИЩЕМ В ОБЛАКЕ !!!
     const event = await FirebaseService.findEventByCode(eventCode);
     
     if (event && (event.status === 'LIVE' || event.status === 'COMPLETED')) {
        setIsJoined(true);
     } else {
  ...
  useEffect(() => {
     if (isJoined) {
        // Подключаемся к эфиру и слушаем обновления игры
        return FirebaseService.subscribeToEverything((game) => {
           setGameState(game);
        });
     }
  }, [isJoined]);
  ...
  const handlePush = () => {
     ...
     setPushCount(p => p + 1);
     // Можно добавить отправку кликов в Firebase, если нужно для Экрана
  };

  const handleGenImage = async () => {
     setIsGen(true);
     const url = await generateAiImage(imgPrompt);
     if (url) setGenImg(url);
     setIsGen(false);
  };
