...
const HostDashboard: React.FC<Props> = ({ activeEvent, setActiveEvent, lang }) => {
  ...
  useEffect(() => {
    // Если событие активно, отправляем его в Firebase, чтобы гости его увидели
    if (activeEvent && activeEvent.status === 'LIVE') {
      FirebaseService.syncEvent(activeEvent);
    }
  }, [activeEvent]); // Срабатывает при любом изменении активного события
  // -------------------------------
  ...
  const handleToggleLive = () => {
    ...
    // Принудительно обновляем облако
  ...
  // Оставил весь код рендеринга без изменений, чтобы сохранить ваш дизайн
  return (
