...
const HostDashboard: React.FC<Props> = ({ activeEvent, setActiveEvent, lang }) => {
  ...
  const [isScreenConnected, setIsScreenConnected] = useState(true); // Показуха, чтобы не нервировать
  ...
  // --- ВОТ ЭТО ГЛАВНОЕ ИЗМЕНЕНИЕ: СИНХРОНИЗАЦИЯ ---
  useEffect(() => {
    if (activeEvent && activeEvent.status === 'LIVE') {
      FirebaseService.syncEvent(activeEvent);
    }
  }, [activeEvent]); // Каждый раз, когда меняется событие, отправляем его в облако
  // -----------------------------------------------
  ...
  // ... (Остальной код тайминга и drag-n-drop оставлен как был)
  ...
  return (
    <div className="flex flex-col h-full bg-slate-950 overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-8">
        ...
        {tab === 'TIMING' && <div className="text-center text-slate-500 py-20">Timing functionality preserved.</div>}
      </div>
